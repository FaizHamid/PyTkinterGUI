...
def forward(image_number):
    ...
    # Update status bar for forward button
    status = Label(root, text="Image " + str(image_number) + " 0f " + str(len(image_list)), bd=1, relief=SUNKEN,
                   anchor=E)
...
def back(image_number):
    ...
    # Update status bar for backward button
    status = Label(root, text="Image " + str(image_number) + " 0f " + str(len(image_list)), bd=1, relief=SUNKEN,
                   anchor=E)
    status.grid(row=2, column=0, columnspan=3, sticky=W + E)
